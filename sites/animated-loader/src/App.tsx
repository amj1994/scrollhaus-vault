...
const darkSrc = (n: string) => `https://qclay.design/lovable/loader/icon-${n}.svg`;
const whiteSrc = (n: string) => `https://qclay.design/lovable/loader/icon-w-${n}.svg`;
...
type Variant = "s" | "c" | "m";
type Hex = { x: number; y: number; variant: Variant; z: number; iconSet?: "dark" | "white"; slot?: number };
...
const HexIconSlot = memo(function HexIconSlot({
  ...
  return (
    <>
      {layers.map((layer) => {
        const src = iconSet === "white" ? whiteSrc(layer.icons[slot]) : darkSrc(layer.icons[slot]);
        const phaseClass =
          layer.status === "incoming" ? "hex-icon-in" : layer.status === "outgoing" ? "hex-icon-out" : "hex-icon-current";
        const zIndex = layer.status === "outgoing" ? 10 : layer.status === "incoming" ? 11 : undefined;
        return (
          <img
            key={layer.id}
            src={src}
...
export function HexScene() {
  ...
  if (currentLayerRef.current == null) {
    currentLayerRef.current = { id: iconLayers[0].id, icons: iconLayers[0].icons };
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Center glow — behind the center active row, above top/bottom c-polygons */}
        <div
          style={{
            position: "absolute",
            top: "50%",
            ...
            height: 120,
            borderRadius: "50%",
            background: "rgba(71, 112, 189, 0.70)",
            filter: "blur(37px)",
      ...
      {/* Loading text and progress bar — outside the masked scene, over the background */}
      <div
        style={{
          position: "absolute",
