...
const cardImage = "https://qclay.design/lovable/kubric/card-image.png";
...
function animateLines(root: HTMLElement, selector: string, baseDelay: number, lineGap: number) {
  const inners = Array.from(root.querySelectorAll<HTMLElement>(selector));
...
function Index() {
  ...
  useEffect(() => {
    ...
    const startAnimations = () => {
      ...
      animateLines(hero, ".hero__heading .hero__line-inner", 0.3, 0.85);
      animateLines(hero, ".hero__label .hero__line-inner", 0.3, 0.65);
      animateLines(hero, ".hero__desc .hero__line-inner", 0.3, 0.65);
    };

    const fallback = window.setTimeout(startAnimations, 5000);
    ...
    if (video) {
      ...
      if (video.readyState >= 4) startAnimations();
    }
    ...
    const anchors = Array.from(hero.querySelectorAll<HTMLAnchorElement>('a[href^="#"]'));
    anchors.forEach((a) => a.addEventListener("click", onAnchorClick));

    const onScrollDown = () => {
      window.scrollTo({ top: window.innerHeight, behavior: "smooth" });
    };
    const scrollDownBtn = hero.querySelector<HTMLButtonElement>("#scrollDown");
    scrollDownBtn?.addEventListener("click", onScrollDown);

    const sideNavLinks = Array.from(hero.querySelectorAll<HTMLAnchorElement>(".side-nav__link"));
    const onSideNavClick = (clicked: HTMLAnchorElement) => {
      sideNavLinks.forEach((link) => {
        const isActive = link === clicked;
        link.classList.toggle("side-nav__link--active", isActive);
        const existingLine = link.querySelector(".side-nav__line");
        if (isActive && !existingLine) {
          const line = document.createElement("span");
          line.className = "side-nav__line";
          link.appendChild(line);
        } else if (!isActive && existingLine) {
          existingLine.remove();
        }
      });
    };
    const sideNavHandlers = sideNavLinks.map((link) => {
      const handler = () => onSideNavClick(link);
    ...
    return () => {
      ...
      scrollDownBtn?.removeEventListener("click", onScrollDown);
      sideNavHandlers.forEach(({ link, handler }) => link.removeEventListener("click", handler));
    };
  }, []);
    ...
    <div className="hero" ref={heroRef}>
      ...
      <div className="hero__blur">
        {Array.from({ length: 8 }).map((_, i) => (
          <div className="hero__blur-layer" key={i} />
        ))}
      </div>
      ...
      <header className="header">
        ...
        <nav className="nav-pill" aria-label="Primary">
          ...
          <a className="nav-pill__link" href="#team">
            Team <span className="nav-pill__badge">3</span>
          </a>
      ...
      <h1 className="hero__heading">
        ...
        <span className="hero__line">
          <span className="hero__line-inner">
            <em className="hero__science">Science</em>
          </span>
        </span>
      </h1>
      ...
      <div className="hero__bottom">
        ...
        <p className="hero__desc">
          <span className="hero__line">
            <span className="hero__line-inner">We enable the world's most engaged investors and</span>
          </span>
          <span className="hero__line">
            <span className="hero__line-inner">family offices to access professionally managed</span>
          </span>
          <span className="hero__line">
          ...
          <div className="about-card__content">
            <div>
              ...
              <p className="about-card__text">
                Me're driven by user-centered design that drives productivity and increases revenue.
              </p>
            </div>
