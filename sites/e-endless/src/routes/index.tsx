...
const barUrls: string[] = [0, 1, 2, 3, 4, 5].map((i) => `${BARS}bar${i}.svg`);

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "E-Endless — The easiest way to build your design products" },
      { name: "description", content: "AI-powered content and design creator for content and marketing teams." },
    ],
  }),
  component: Index,
});
        ...
        <div className="relative">
          ...
          <div className="relative overflow-hidden mx-4 mb-0 border border-white/10 rounded-2xl flex flex-col items-center text-center pt-[64px] px-6 pb-0">
            ...
            <p className="relative z-10 text-base sm:text-2xl opacity-60 text-neutral-100 w-[634px] max-w-full leading-snug mb-[24px] sm:mb-[30px] word-stagger">
              <StaggeredWords text="E-Endless designer — the AI-powered content and design creator for content and marketing teams." baseDelay={900} step={33} />
            </p>
            <div className="relative z-10 w-[572px] max-w-full h-12 mb-[25px]">
              <div className="absolute inset-0 bg-neutral-900 outline outline-[1.30px] outline-white/10 rounded-xl flex items-center px-4 gap-3 anim-reveal-right" style={{ animationDelay: "1620ms", clipPath: "inset(0 100% 0 0)" }}>
                ...
                <TypingPlaceholderInput placeholder="Write your prompt/" startDelay={2040} speed={70} />
        ...
        <div className="max-w-7xl mx-auto px-2 py-16 flex flex-col gap-24">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-10 items-start">
            <div className="md:col-span-4 flex items-center gap-4">
              ...
              />
              <span className="text-3xl font-medium text-neutral-100" aria-label="E-Endless">
...
function StaggeredWords({ text, baseDelay = 0, step = 90, active = true, groupSize = 1 }: { text: string; baseDelay?: number; step?: number; active?: boolean; groupSize?: number }) {
        ...
        >
          <p className="text-4xl text-white leading-snug">
            ...
            <span className="relative inline-block align-baseline px-2 py-1">
              ...
              {/* black overlay text revealed in sync with bar */}
              <motion.span
                aria-hidden
...
function useInView<T extends HTMLElement>(rootMargin = "0px 0px -15% 0px") {
...
function TypingPlaceholderInput({ placeholder, startDelay = 0, speed = 70 }: { placeholder: string; startDelay?: number; speed?: number }) {
...
function Pill({ label, icon, bg, text, iconBg, invertIcon = false }: { label: string; icon: string; bg: string; text: string; iconBg: string; invertIcon?: boolean }) {
  return (
    <div
...
function CountUpDelayed({ end, delay, active, duration = 1800 }: { end: number; delay: number; active: boolean; duration?: number }) {
  ...
  return <CountUp end={end} duration={duration} active={start} />;
}
...
function RevealItem({ active, delay, type = "rise", children, className, style }: { active: boolean; delay: number; type?: "rise" | "pop"; children: React.ReactNode; className?: string; style?: React.CSSProperties }) {
  const anim = type === "rise"
    ? "rise-up 1s ease-out forwards"
    : "pop-in 0.7s cubic-bezier(0.22,1,0.36,1) forwards";
  return (
    <div
      className={className}
      style={{
        ...style,
        opacity: active ? undefined : 0,
        animation: active ? anim : undefined,
