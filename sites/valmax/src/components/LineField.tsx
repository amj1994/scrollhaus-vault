...
type Marker = { lineIndex: number; t: number; label: string };
...
function pointAt(line: Line, t: number) {
    ...
    >
      ...
      {markers.map((m, i) => {
        ...
        return (
          ...
          >
            <circle cx={x} cy={y} r={0.18} fill="#fff" opacity={0.95} />
            <text
              x={x + 0.5}
              y={y - 0.6}
              fill="#fff"
            ...
            >
              {words.map((w, wi) => (
                <tspan key={wi} x={x + 0.5} dy={wi === 0 ? 0 : "0.7em"}>
                  {w}
                </tspan>
              ))}
            </text>
