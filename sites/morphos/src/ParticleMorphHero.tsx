...
export function generateBoneSynapses(tempBonePositions: number[], totalBonePoints: number) {
  ...
  for (let i = 0; i < numBoneSynapses; i++) {
    ...
    const c = new THREE.Color('#fe1b40'); // Glowing bright neon red
    ...
    if (dSeed < 0.5) {
      dc.set('#fffeff'); // brilliant white sparkle cores
    } else if (dSeed < 0.85) {
      dc.set('#fe1b40'); // vivid red
    } else {
      dc.set('#fe8192'); // light coral red
    }
...
export function generateBoneConnections(boneSynapses: BoneSynapse[]) {
  ...
  for (let i = 0; i < boneConnections.length; i++) {
    ...
    const dSeed = Math.random();
    if (dSeed < 0.5) {
      dc1.set('#fffeff'); // high-contrast white-red lines
      dc2.set('#ff0000');
    } else {
      dc1.set('#fe1b40');
      dc2.set('#fe8192');
...
export function generateSkullConnections(skullSynapses: SkullSynapse[]) {
  ...
  for (let i = 0; i < skullConnections.length; i++) {
    ...
    const c1 = new THREE.Color('#7a2dbe');
...
export default function ParticleMorphHero() {
  ...
  useEffect(() => {
    ...
    // FIX: Reduced strength/radius/threshold so bloom no longer floods the hollow ring center
    const renderPass = new RenderPass(scene, camera);
    const bloomPass = new UnrealBloomPass(
      new THREE.Vector2(width, height),
      0.55, // Strength (was 1.4 - too strong, was flooding center)
      0.18, // Radius (was 0.4 - too wide, was bleeding into hollow area)
      0.35  // Threshold (was 0.1 - too low, made everything bloom)
    );
    ...
    // 5. Helper function to generate glowing 2D diamond texture (making particles look like cut diamonds)
    ...
    // FIX: A tighter, crisper glowing diamond ring texture
    ...
    // 6. 3D GLOWING CYBER-MASK (Restoring original cyber-mask)
    ...
    for (let rIndex = 0; rIndex < rows; rIndex++) {
      ...
      for (let cIndex = 0; cIndex < cols; cIndex++) {
        ...
        // Add a tiny bit of high-tech organic jitter to make it feel like floating nanotech
        const jitter = 0.015;
        ...
        if (isNearEyeBorder) {
          ...
          else if (blend < 0.7) c.set('#00fefe');
          else c.set('#01aaff');
        }
    ...
    // 6b. 3D GLOWING CYBER-BRAIN (Replacing 3D cyber-mask for Phase 2)
    // Generating a highly detailed 3D brain model with surface folds, a stem, glowing synapses, and connected neural lines.
    ...
    const applyFaceMolding = (x: number, y: number, z: number) => {
      ...
      if (z > 0) {
        ...
        // 1. Nose bridge and tip sculpting (centered horizontally, middle vertically)
    ...
    // 1. CEREBRUM (Winding surface folds, left/right hemispheres, front-to-back lobe profiling)
    for (let i = 0; i < numSurfacePoints; i++) {
      ...
      // Lateral/Sylvian fissure indentation (on the sides of the cerebrum)
      ...
      // Self-illuminated dual-color styling matching the attached image (glowing cyan peaks, deep blue crevices)
      const c = new THREE.Color();
      const dc = new THREE.Color();
      const dSeed = Math.random();
      ...
      if (foldVal > 0.08) {
        ...
        if (blend < 0.6) {
          ...
          c.set('#a7fefe'); // white-cyan hot spot
        }
        if (dSeed < 0.7) {
          dc.set('#fffeff'); // dazzling white core
        } else {
          dc.set('#eaffff'); // shimmering cyan-white
        }
        ...
        if (dSeed < 0.5) {
          dc.set('#b6f7fe'); // crystal clear ice blue
        } else {
          dc.set('#6bfcff'); // bright diamond cyan
        }
      } else {
        ...
        if (dSeed < 0.5) {
          dc.set('#0189fe'); // royal diamond blue
        } else if (dSeed < 0.8) {
          dc.set('#e9ccff'); // prismatic violet
        } else {
          dc.set('#1a3dfc'); // deep sapphire refraction
        }
      }
    ...
    // 2. CEREBELLUM (Lower-back lobe nested underneath, styled with dense horizontal striations/folia)
    ...
    // 3. BRAIN STEM (Faceted hexagonal crystal column tapering to a sharp lower point)
    ...
    for (let i = 0; i < numStemPoints; i++) {
      ...
      if (dSeed < 0.4) {
        ...
        dc.set('#e0cdff'); // crystal lavender
      }
      tempBrainColors.push(c.r, c.g, c.b);
      tempDiamondColors.push(dc.r, dc.g, dc.b);
    }
    ...
    for (let i = 0; i < numSynapses; i++) {
      ...
      const c = new THREE.Color('#fffeff'); // High intense glowing synapse
      ...
      const dSeed = Math.random();
      if (dSeed < 0.6) {
        dc.set('#fffeff'); // hot crystal white
      } else if (dSeed < 0.8) {
        dc.set('#b7feff'); // brilliant electric cyan
      } else {
        dc.set('#f2d9fe'); // glittering prismatic pink-lavender
      }
    ...
    for (let i = 0; i < connections.length; i++) {
      ...
      const c1 = new THREE.Color('#00fefe');
      const c2 = new THREE.Color('#0055fe');
    ...
    // Grouping all brain elements (surface, stem, synapses, paths) together so they scale & rotate as one unit
    const brainGroup = new THREE.Group();
    brainGroup.add(brainPoints);
    brainGroup.add(synapsePoints);
    brainGroup.add(neuralLines);

    // Set initial layout and three-quarters perspective matching the attachment
    ...
    // 7. OUTER SCATTERED CLOUD (rotates on Y-axis, this rotation is fine for a 3D scattered cloud)
    ...
    const coreColor = new THREE.Color('#00fec9');
    const outerColor = new THREE.Color('#072d30');
    const maxRadius = 15;
    // FIX: hollow gap - nothing spawns closer than the brain + a margin, so the center/brain interior stays dark
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      if (distributionSeed < 0.55) {
        // Structured halo/ring band (Gaussian) - sits well outside the hollow gap
        const u1 = Math.random() || 0.0001;
        const u2 = Math.random();
        const stdNormal = Math.sqrt(-2.0 * Math.log(u1)) * Math.sin(2.0 * Math.PI * u2);
        r = maxRadius * (0.55 + stdNormal * 0.08);
      } else {
        // Soft outer ambient cloud, still starts beyond the hollow gap
      ...
      const dSeed = Math.random();
      if (dSeed < 0.1) {
        dc.set('#fffeff'); // brilliant white sparkle
      } else if (dSeed < 0.4) {
        dc.set('#c9faff'); // icy diamond blue
      } else if (dSeed < 0.7) {
        dc.set('#00ffd4'); // bright cyan
      } else if (dSeed < 0.85) {
      ...
      if (aSeed < 0.1) {
        ac.set('#fffeff'); // brilliant white sparkle
      } else if (aSeed < 0.4) {
        ac.set('#fe4c6c'); // glowing pinkish red
      } else if (aSeed < 0.7) {
        ac.set('#fe003c'); // vibrant crimson red
      } else if (aSeed < 0.85) {
        ac.set('#cd0101'); // deep red
      } else {
      ...
      const pSeed = Math.random();
      if (pSeed < 0.1) {
        pc.set('#fffeff'); // brilliant white sparkle
      } else if (pSeed < 0.4) {
        pc.set('#e1aaff'); // glowing lavender orchid
      } else if (pSeed < 0.7) {
        pc.set('#9d4edd'); // neon violet/purple
      } else if (pSeed < 0.85) {
        pc.set('#ff00ab'); // vibrant magenta
      } else {
        pc.set('#10011a'); // deep dark celestial space purple-black
      }
    ...
    const animate = () => {
      ...
      if (brainProgress > 0.01) {
        const baseRotY = -Math.PI / 4.5; // Three-quarters profile view showing beautiful face contours!
        const baseRotX = 0.08;
  ...
  return (
    <div className="relative w-screen h-[750vh] bg-[#010001] select-none">
      <div className="fixed top-0 left-0 w-full h-screen overflow-hidden">
        ...
        {/* Responsive Overlay Texts using 'Anybody' font and matching attachment layout */}
        <div
          ref={overlayRef}
        ...
        >
          ...
          <div className="flex justify-between items-start w-full mt-4 sm:mt-0">
            <div
              ...
              className="text-[30px] leading-[30px] tracking-tight"
            >
              The World
            </div>
            ...
            >
              {['Products', 'Features', 'Technology', 'Support', 'Pricing'].map((item) => (
                <div key={item} className="flex items-center space-x-2 md:space-x-3 justify-end hover:text-[#00ffd0] transition-colors cursor-pointer pointer-events-auto">
                  <span>{item}</span>
                  <span className="text-[#00fefe] font-bold">•</span>
                </div>
              ))}
            </div>
          </div>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-end w-full">
              ...
              <div className="max-w-md md:ml-auto">
                ...
                >
                  Neural nodes reconstruct into a perfect human face geometry, representing the ultimate mathematical synthesis of form and mind.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-end w-full">
              ...
              <div className="max-w-md md:ml-auto">
                <p
                  ...
                  className="text-[14px] sm:text-[16px] md:text-[18px] leading-[22px] sm:leading-[26px] md:leading-[30px] tracking-normal pointer-events-auto"
                >
                  The fundamental skeletal lattice aligns under gravity. A resilient structural blueprint emerges to shield the vital essence of life.
                </p>
              </div>
            </div>
          </div>
          ...
          <div
            ref={skullTextRef}
            className="absolute bottom-6 sm:bottom-12 md:bottom-16 left-6 sm:left-12 md:left-16 right-6 sm:right-12 md:left-16 flex justify-start items-end mb-4 sm:mb-0 transition-opacity duration-150"
            style={{ opacity: 0, transform: 'translateY(40px)', pointerEvents: 'none' }}
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-end w-full">
              {/* Left Side: Header */}
              <div className="max-w-xl">
                ...
                <span className="block text-xs uppercase tracking-[0.3em] text-[#c67dfe] mt-4 font-mono font-semibold pointer-events-auto">
                  The Human Cranial Skeleton
                </span>
              </div>
              ...
              <div className="max-w-md md:ml-auto">
                <p
                  ...
                  className="text-[14px] sm:text-[16px] md:text-[18px] leading-[22px] sm:leading-[26px] md:leading-[30px] tracking-normal pointer-events-auto"
                >
                  The majestic cranial architecture shields our consciousness and houses our thoughts. Within this secure neural dome, memory and form merge into an intricate cosmic crown of glowing starlight.
                </p>
              </div>
            </div>
          </div>
